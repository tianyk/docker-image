...
prepare_configuration() {
    ...
    if [ -n "${JOBMANAGER_MEMORY_PROCESS_SIZE}" ]; then
        set_config_option jobmanager.memory.process.size ${JOBMANAGER_MEMORY_PROCESS_SIZE}
    fi

    if [ -n "${TASKMANAGER_MEMORY_PROCESS_SIZE}" ]; then
        set_config_option taskmanager.memory.process.size ${TASKMANAGER_MEMORY_PROCESS_SIZE}
    fi
